    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group" >
                        ...
                        <form action="{{url('/cek2')}}" method="get" >
                    <label class="font-weight-bold">Ongkos Kirim</label>
                    <input type="teks" class="form-control"   name="name" placeholder="Cari Kecamatan/Kabupaten/Kota" >
                    <button class="btn btn-secondary" style="margin-top: 15px; margin-bottom: 20px; margin-left: 10px;" type="submit">Cari</button>                    
            </form>
            ...
            <div class="col-md-12 form-group p_star" >
                        ...
                        <select class="form-control" id="exampleFormControlSelect1">
                    @foreach ($responseBody['pricing'] as  $kurir)
                                <option value="{{ $kurir['courier_name'] }} {{ $kurir['courier_service_name'] }} {{ $kurir['price'] }}    ">
                                {{ $kurir['courier_name'] }} 
                                {{ $kurir['courier_service_name'] }}
                                {{ $kurir['price'] }}
                                </option>
                            @endforeach
    </select>
            ...
            if (provindeId) {
                jQuery.ajax({
                    ...
                    success: function (response) {
                        $('name="bandung"').empty();
                        $('name="bandung"]').append('<option value="">-- pilih kota asal --</option>');
                        $.each(response, function (key, value) {
                            $('name="bandung"').append('<option value="' + key + '">' + value + '</option>');
                        });
                    },
                });
            } else {
